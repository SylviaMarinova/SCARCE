...
for key, i in dict_trade_barriers.items():
    i['Import*ETI'] = ETI['ETI 2016'] * i['Percentage'] #Multiply the ETI an the imported percentage
    i.reset_index(inplace = True)
    i.loc['Result'] = pd.Series(
        i['Import*ETI'].sum(),
        index = ['Import*ETI']
...
supply_risk_scaled_final.to_csv(
    'Results/Results 19.01.22/Supply risk/Supply risk scaled.csv', index = True
    )
vulnerability_final.to_csv(
    'Results/Results 19.01.22/Vulnerability/Vulnerability.csv', index = True
    )

social_standard_final.to_csv(
    'Results/Results 19.01.22/Social standards/Compliance with social standards.csv', index = True
    )
environmental_standard_final.to_csv(
    'Results/Results 19.01.22/Environmental standards/Compliance with environmental standards.csv', index = True
    )

#%%Export the results with total column

supply_risk_scaled_final_total.to_csv(
    'Results/Results 19.01.22/Supply risk/Supply risk total.csv', index = True
    )
vulnerability_final_total.to_csv(
    'Results/Results 19.01.22/Vulnerability/Vulnerability total.csv', index = True
    )

social_standard_final_total.to_csv(
    'Results/Results 19.01.22/Social standards/Compliance with social standards total.csv', index = True
    )
environmental_standard_final_total.to_csv(
    'Results/Results 19.01.22/Environmental standards/Compliance with environmental standards total.csv', index = True
    )

############## PLOT THE CRITICALITY MATRIX ################

results = pd.read_csv(
    '/Users/sylvia/Desktop/Work/SCARCE/Python/Results/Results 19.01.22/Result_total.csv'
    )
